...
> `/pj:setup` `/pj:spec` `/pj:design` `/pj:build` `/pj:change` の **5スキル共通の唯一の出典**。
> 起動したらまずこれを読む。各スキル固有の振る舞い（何を生成するか・どんな質問をするか）は各 SKILL.md にある。
>
> **この文書は「毎回読む分」だけを持つ。** 特定の場面でしか要らない詳細は外に置く（節番号は変えない）:
>
> | 読むのは | ファイル |
> |---|---|
...
仕様駆動パイプライン。3フェーズは別々のツールではなく、**同じ語彙・同じ評価軸・同じ盤面・同じ保守規律で
1つの真実を育てる一体のファミリー**。**その3つに直交して `/pj:setup` が居る。**
...
**setup はフェーズではなく、決定が起きるたびに呼ばれる。** spec の途中で「これは package だ」と分かったら
器を作り（`package`）、design で技術が決まったらインストールして配線し（`stack`）、docs と実体がズレたら
埋める（`sync`）。**中身は作らない**——器と実体だけを用意して引き渡す。

- **なぜ分けるか**: 「Next.js を入れる」は特定の feature に属さないので build に置けず、docs しか書かない
  design にも置けない。**決定と実体の間には必ず一手ある。**
- **docs が先に進み実体が後から追いつくのは正常。** ズレたまま放置されないよう `/pj:setup sync` で追いつく。

> **setup はモデル（Claude）からも起動できる**（モードごとの承認は `skills/setup/SKILL.md` の
> 「モデル起動時のゲート」が正）。**「器が要る」と気づくのは会話の途中で、そこに居るのはモデル**だから——
> ここでコマンド入力を要求すると、**docs だけ手で書かれて器が永久に生まれない**。
>
> **それでも次は義務のまま残る**（気づいた者が黙って回避しない。これが唯一の担保）:
>
...
**`docs/specs/overview.md` を持つディレクトリが product。** 探索は `docs/specs/overview.md` ＋
`packages/*/docs/specs/overview.md` ＋ `packages/*/*/docs/specs/overview.md` の3箇所。

- **`packages/` 直下はフラットが既定。** 並びを揃えたいだけなら**ディレクトリ名の接頭辞**で束ねる
  （`adapter-slack/` `adapter-line/`）。**先回りしてグループを作らない。**
- 数が増えて束ねたくなったら、**グループ用の中間ディレクトリを1段だけ**挟んでよい
  （`packages/<group>/<name>/`）。**グループ自身は product ではない**（overview.md を持たないので
  探索に引っかからない。ただのフォルダ）。**深さは `packages/` 配下2段まで。**
- **product の中に product を置かない**（`packages/a/packages/b`）。依存グラフと切り出しの単位が曖昧になる。
...
> **1. 語彙** — 別の app がこの package をそのまま使えるか。**人が判断する。**
> **2. 可搬** — 別のリポジトリにコピーして `bun install` し、そのまま型検査とテストが通るか。**実行して確かめる。**

**両方を満たして初めて独立している。** 語彙が完全に一般的でも、親の tsconfig を extends し、テストランナーを
root から借りていれば、外に出した瞬間に動かない。逆に物理的に自己完結していても、app 固有の制度語を持てば
別の app では使えない。

**語彙の線引き**: **package は自分の責務のドメイン語彙を持ってよい**（組織図ライブラリが「組織」「人」を
知るのは当たり前で、抽象化しても表現力が増えず包む層が増えるだけ）。**禁じられるのは app 固有の語**
——その app の制度・他 feature の概念に依存した語。**判定は「別の会社の同種システムがそのまま使えるか」**。
機械では決まらないので terminologist の判断に委ねる（§14）。

**可搬は最初から満たす。** workspace の hoisting が不足を隠すので、**壊れていても静かに壊れる**。

> **チェックリスト・分割の基準・強制の4枚は `_shared/product-model.md`。**
> package を作る／切り出す／監査するときに読む。

## 3. 1つの真実の3レイヤー（トレース可能）

**product ごとに `docs/` を持ち、それがその product の単一の真実。下は上から導出され、必ず上へ辿れる。**

> **以下すべてのパスは「対象 product のルートからの相対」**（root product なら repo 直下、package product なら
...
`docs/design/design-language.md`（視覚の正・DL-NN）。package はこれらを**参照するが所有しない**
（package ごとに視覚の正を持つと DS が割れる）。**`docs/design/intake/` は product ごと**（visual.md §V2）。

**トレースの鎖（保守の基盤）**:

- `受入条件（spec） ↔ test ↔ code` ／ `design 決定 ↔ コードモジュール / スキーマ` ／
  `DL-NN ↔ コンポーネント`（visual.md §V1）
...
鎖を「意味解釈なしに grep で機械検証」できる状態に保つための規約。
...
- **一度振った ID は変えない。** 廃止したら**欠番**にし、**再利用しない**。欠番はその旨と廃止理由を1行残す。
...
- **受入条件だけ「新規」と「増減」で採番者が分かれる。** 判定は「**この feature に ID 付きの受入条件が
  1つでもあるか**」——**無ければ `/pj:spec`（書いた直後にID未採番のドラフトを残さないため）、
  あれば `/pj:change`**（欠番管理と下流への伝播が要るので採番者を1人に保つ）。
- 鎖の検証は**どの ID も同じ形**: 「全 ID に参照側が存在するか」を grep で突合する（未参照 = 未実装）。
- **ID を1つ増やしたら、この表に1行足すだけで済むようにする。** 他所は種類を列挙せず「§4 の安定 ID」と書く。
- **feature 名は product をまたいで repo 内で一意にする。** AC ID に product 名を含めないので、同名 feature が
  2つあると `grep 'workflow-AC-'` が別 product の鎖を拾って検証が壊れる。prefix を付けないのは
  **package を切り出したとき ID をそのまま生かす**ため。衝突は audit が検出する（§14）。
...
- **4-5 をつけてよいのは「追加対話が不要」と言い切れるときだけ。** これから「詰める／深掘りする」と
  話している成果物は最大 3。**枠・構造が決まっただけでは 3 どまり。**
- **ラチェットではない。毎回つけ直す。** 論点が増えた・前提が覆ったら**遠慮なく下げる**（後退は変更履歴に残す）。
- **必ず frontmatter に保存し、表示はその保存値から出す。** 表示のたびに推定し直すと値が揺れて、
  下がったことに気づけなくなる（「毎回つけ直す」は**保存値を更新する**という意味）。
- **readiness は product ごとに閉じて測る。** 他 product の未決を持ち込まない
  （持ち込むと「この package は独立して作れるか」が測れなくなる）。
- **design 全体の確定度 = 上表の各ファイルの `progress` の平均**（UI が無ければ design-language を除く。
  **package は root 所有の conventions・design-language を平均に含めない**——自分で動かせない値を混ぜると
  他 product の都合で自分の数字が動く）。
- バー表記（■と□）を正とする。数値は補助。
...
- **全 product に `features/` を強制しない。** 小さな package は feature 1個分の中身しかなく、器だけが増える。
- **段階1でも AC ID は必ず振る**（§4）。鎖は器の形ではなく ID で成立する。**切り出しても ID は変えない**（移動するだけ）。
- 判断基準は「1ファイルが読みにくくなったか」。**先回りして割らない。** root product も同じ。
...
5. **軽量修正** — 既存成果物への一点修正のうち、**§4 の安定 ID を増減しないもの** → 即時反映（§8）。
   **ID の増減を伴うものはここで処理せず `/pj:change` に送る**（緩めると ID の無い受入条件・原則が生まれ、
   §4 の鎖が機械検証できなくなる）。**ID の種類をここで列挙しない**（正は §4 の表）
...
起動できる**（`build` はコマンド必須）。モデル起動時の前提チェックと承認の取り方の**正は各 SKILL.md 冒頭の
...
`/pj:change` が方向と正レイヤーを判定して以下を**開発者に代わって実行する**。覚えるのは「直したい → `/pj:change`」だけ。
...
3. そのファイル末尾の `## 変更履歴` に `YYYY-MM-DD: <変更内容>` を追記 → 4. frontmatter の `updated` を今日に
...
**上段（導出）= root product の `docs/specs/overview.md` の product 表** —— 全 product の一覧。
...
コードの識別子も同じ語に従う**。用語の一意性は実装の誤りを防ぐ最重要要素。監査は terminologist（§14）。

**なぜ1つにまとめないか**: 1ファイルだと app 固有の語と package の語が並び、**どちらの責務の語かが
判別できなくなる**。product ごとに分ければ**語彙の帰属が物理的にはっきりする**。
...
UI の正は **`_shared/visual.md`** が持つ（**§V1** design-language ＝ 視覚の正と DS の3スロット契約 ／
**§V2** intake ＝ 外部の設計成果物の取り込み）。**読むのは UI に触れるスキルだけ**——`/pj:setup`（DS 導入）・
`/pj:design`・`/pj:build`（UI 実装時）・`/pj:change`（UI に触れる変更時）。**`/pj:spec` は読まない**
（WHAT に視覚は含まれない）。CLI 等 UI の無いプロジェクトでは誰も読まない。ID 規約（DL-NN）の正は §4。
...
- **判定は「テストできるか」1つ。** 数値だから見た目、ではない。「行高 39px」は検証できるので挙動、
  「影を使わない」は全画面に効くので見た目。
- **「まとめる」に ID が無いのが要**。ここは自分で決めることが無い。**新しい判断が生まれたら、それは挙動か
  見た目のどちらかに属するはず**——という検査になる（アダプタの当て方は product-model.md）。

**置き場は conventions が持ち、build は引くだけ。** 具体的なディレクトリの表は root の
`docs/design/conventions.md`（§13）。pj はディレクトリ名を規定しない（repo と FW で違う）。規定するのは3つ:
...
- 作法に従うことは**依存ではない**。package を切り出すときは、baked-in の設計判断としてその package の
  `docs/design/` に写して連れて行く。
...
委譲の作法: sub-agent を起動し、**対象（docs パス / 特定 feature）とレイヤー・product 種別**を渡す。
返った結果は**丸投げせず要点を整理**して伝える（重大な指摘を先頭に）。review/audit 自体では**編集しない**。
...
- agent は **`pj:drift-auditor`**。**監査観点の正はその agent が持つ**（ここには書かない）。
- 対象は**対象 product のトライアド**（`docs/specs/` ＋ `docs/design/` ＋ `src/`）＋ root の
  `conventions.md`・`design-language.md`。**呼び出し元の層に関わらず常にこの3点セット**
  （spec だけ渡すと鎖の切れ目が対象外に落ちる）。
- **product が明示されなければ全 product を順に。** 特定 feature に絞るときも対象はトライアドのまま、
  **スコープとして feature 名を渡す**。
- 報告は**product 境界の違反を先頭**に（境界が壊れていると鎖の議論が意味を失う）。呼び出し元の層に応じて
  その次の並び順だけ変えてよい。**編集しない**（直すのは `/pj:change`）。
- UI の視覚違反・取り込んだ出典との乖離を深掘りするときのみ **`pj:design-reviewer`** を併用する。
...
- **package の spec / glossary に app 固有の語彙を持ち込む**（ただし自分の責務の語彙は違反ではない・§10）
- **「全 package 共通の◯◯」を app の spec に書く**（作法は root の conventions・§13）
- **上段ダッシュボードを手で動かす**（下段が正・§9）／**先回りして `features/` に割る**（§6）
...
- **L0 コスメであっても design-language（DL-NN）に準拠する**。
- 判定手順・product の特定・採番の実務は **`skills/change/SKILL.md`** が持つ（この表の適用先）。

## 17. プロジェクトへの運用宣言（managed block）

pj で運用するプロジェクトは、**ルート `CLAUDE.md`（毎セッション常時ロード）に運用宣言を持つ**。
`concepts.md` 等のプラグイン内部文書は skill 実行中しか読まれないため、宣言が無いと**普通のセッションや
新しい人が入口（`/pj:change`）に辿り着けない**。そこで宣言を**プラグインが所有・生成する管理ブロック**として
焼き込む（防御: ルールが向こうから来る）。
