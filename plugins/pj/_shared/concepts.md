...
### 「独立して作れる」の定義 — 語彙と可搬の2つで測る

**判定は2つ。両方を満たして初めて独立している**:

> **1. 語彙** — 別の app がこの package をそのまま使えるか。
> **2. 可搬** — 別のリポジトリにコピーして `bun install` し、そのまま型検査とテストが通るか。

**1 は人が判断し、2 は実行して確かめられる。** 片方では足りない。語彙が完全に一般的でも、
親の tsconfig を extends し、テストランナーを root から借りていれば、外に出した瞬間に動かない。
逆に物理的に自己完結していても、app 固有の制度語を持てば別の app では使えない。

#### 可搬（機械で測れるほう）

**最初から満たす。「切り出すときにやる」ことではない。** workspace の hoisting が不足を隠すので、
**壊れていても静かに壊れる** —— 実際に切り出すまで誰も気づかない。後から剥がすのは、
root に寄せた設定を1つずつ引き剥がす作業になる。
...
- **`bun run type-check` と `bun run test` が package 単体で通ることが完成条件。**
  root から一括で走らせて通るのは、可搬の証明にならない。
- **判定は自動化できる。** 対象 package を空ディレクトリにコピーし、`bun install` して
  上の2コマンドを走らせる。通らなければ独立していない。
...
**「コアと UI の2つ」のような固定の型を持たない。** 割る数は責務の数ではなく、
**下の基準に当てはまった数**で決まる。当てはまらないなら1つのままでよい。

> **迷ったら割らない。** 1 package の中を `src/<関心>/` に分け、**lint で依存方向を強制**すれば、
> 境界は package を増やさなくても敷ける（Nx の module boundaries と同じ考え方）。
> 後から割るのは簡単だが、**むやみに増やした package を統合するほうが高くつく**。
...
第三者が実装を書く    利用者が自分の実装（プロバイダ・アダプタ）を足すことを想定するなら、
                      **契約（型とインターフェース）をコア本体から分ける**。
                      契約だけに依存すれば実装が書ける状態にする
                      （実例: AI SDK のプロバイダは @ai-sdk/provider だけに依存し、
                        コア本体 ai には依存しない）

利用者が選んで入れる  複数の実装が入れ替え可能で、利用者が使うものだけ入れたい
...
- **「層だから」** —— 層は依存の方向の話であって、パッケージの数の話ではない。
  方向は lint で守れる
- **「型だけだから軽い / 重い」** —— 型は実行時に消えるが、それは分割の理由にも
  分割しない理由にもならない。判断は上の基準で行う
- **常に一緒に変わるもの** —— 境界のコストだけかかって利得がない

**構造の作法**（広く使われている形に合わせる）:

- **`packages/` 直下はフラット。中間段（`packages/providers/<name>/`）を作らない。**
  ツールによっては入れ子を解決できない。並び順を揃えたいなら
  **ディレクトリ名の接頭辞**で束ねる（`adapter-slack/` `adapter-line/`）
- **公開名とディレクトリ名は一致しなくてよい。** 並びやすいディレクトリ名と、
  読みやすい公開名は別の都合で決まる
- **パッケージを増やさずに関心を分ける手段としてサブパス export がある**
  （`<pkg>/events` のように、1 package の中で入口だけ分ける）。
  版を分ける必要が無いならこちらで足りる
...
4. **可搬を実際に試す** —— package を空ディレクトリへコピーして `bun install` し、
   `type-check` と `test` を走らせる。1〜3 は「app を知らない」を守るが、
   **可搬はそれとは別の境界**で、1〜3 を全部通っても落ちる（実例: 依存グラフは完璧なのに
   vitest を1つも宣言しておらず、root の設定を借りていた）。
...
**「アダプタだから まとめる」ではない。**同じ判定（テストできるか）を当てる。
...
- **外部サービスに繋ぐアダプタは、たいてい「挙動」になる。** 相手の仕様に合わせた
  判断がそこに入るため。「繋ぐだけ」に見えても、**落ちたときにどうするかを決めているなら挙動**
- product として切り出したアダプタが「まとめる」だと、**AC を持てない product** という
  矛盾になる（§2 は product に AC を要求する）。その場合は分類を疑う前に
  **「本当に product にする必要があるか」を疑う**（上の「いくつに割るか」）
- 良い設計は自然にこの形になる。例:
