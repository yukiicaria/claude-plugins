...
> **setup はユーザーが打つ（`disable-model-invocation`）。** 重い工程を勝手に走らせないための設計だが、
> **「器が要る」と気づくのは会話の途中で、そこに居るのはモデル**という非対称が生まれる。
> ここを放置すると、モデルが docs だけ手で書いて器が永久に生まれない。**代わりに次を義務にする:**
>
...
**起動経路とモードの関係**: ユーザーが `/pj:*` と打つ従来の起動に加え、**spec / design / change の3つは
モデル（Claude）判断でも起動できる**（`build` / `setup` はコマンド必須のまま）。モデル起動のときは
**status / next / review / audit は無確認、編集系はユーザー承認が要る**。前提チェック（pj 運用宣言の有無）と
