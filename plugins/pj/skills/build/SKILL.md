...
build は spec・design の**下流**。実装に入る前に必ず両方を読む:
- `docs/specs/`（WHAT — 特に対象 feature の受入条件）
- `docs/design/`（HOW — stack / data-model / conventions / 関連 ADR。これが拘束条件）

**UI を実装するときは `../../_shared/visual.md` も読む**（§V1 視覚の正 / §V2 モック）。
UI に触れない feature（API・バッチ等）では読まなくてよい。

WHAT でも HOW でも、**変えたくなったら build で辻褄を合わせず `/pj:change` に投げる**（concepts.md §4）。
...
6. **同期**: 全受入条件テストが緑になったら
   - 対象 feature の `build_progress` を更新（concepts.md §2。5 = 全受入条件テスト緑）
   - overview の2軸ダッシュボードの build 列を更新（**進捗の正はここ1箇所**。コア機能リストは触らない）
...
build で勝手に辻褄を合わせない。**`/pj:change <気づいたこと>` に投げる**（concepts.md §4 典型シナリオ1）。
...
- review はコード品質の確認。専用 agent を作らず**既存の `/code-review` `/security-review` `/verify`
  に委譲**する（再発明しない）。
- audit は **concepts.md §7「audit の起動仕様」に従う**（全 skill 共通の1つの監査。対象は常に
  `docs/specs/` ＋ `docs/design/` ＋ `src/`）。実装と仕様/設計の乖離を報告。編集しない。

## 進捗管理

各 feature の `build_progress`（build readiness）を concepts.md §2 の軸で測る。基準は明快:
